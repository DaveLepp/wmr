...
public class HadoopEngine implements JobEngine
{
	...
	private static class TaskLog
	{
		...
		public String getURL(boolean error)
		{
			...
			return trackerHttp + "/logs/userlogs/" + attemptID.toString() + "/" + filter;
			//old path
		...
		for (TaskLog log : logs)
		{
			try
			{
				...
				if (!StringUtils.isBlank(logContents))
					return logContents + "\nElapsed ms: " + executionTime;
			}
			catch (Exception ex)
			{	
				return "[Error retrieving task log at " + log.getURL(true) + "]: Execution time: " + executionTime;
			}
		}
